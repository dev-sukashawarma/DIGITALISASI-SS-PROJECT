...
async function checkEmpangOrders() {
  const { data: outlets } = await supabase.from('outlets').select('id, name')
  const outlet = outlets?.find(o => o.name.toLowerCase().includes('empang'))

  if (!outlet) {
    console.log("Outlet Empang not found. Available outlets:")
  ...
  console.log(`\n=== PESANAN TIKTOK GO DI EMPANG TANGGAL 13 AGUSTUS 2026 ===`)
  if (!orders || orders.length === 0) {
    ...
    console.log(`\nGRAND TOTAL EMPANG: Rp ${grandTotal.toLocaleString('id-ID')}`)
