# Scope: Milestone 1 - Implementation Track
...
|---|------|-------|-------------|--------|
| 1 | Wait Screen & Dashboard | Implement `app/attendance/page.tsx` (Wait screen) and update `/kasir` dashboard to display cashier name and branch. Verify `app/api/attendance/webhook/route.ts` and `mock-attendance.js` work as expected. | none | IN_PROGRESS |
