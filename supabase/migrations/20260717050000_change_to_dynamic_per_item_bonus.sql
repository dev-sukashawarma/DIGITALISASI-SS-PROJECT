-- 20260717050000_change_to_dynamic_per_item_bonus.sql

-- 1. Rename bonus_amount to per_item_bonus
ALTER TABLE public.daily_sales_targets RENAME COLUMN bonus_amount TO per_item_bonus;

-- 2. Drop the old function
DROP FUNCTION IF EXISTS public.resolve_daily_bonus(UUID, DATE);

-- 3. Create the new resolve_per_item_bonus
CREATE OR REPLACE FUNCTION public.resolve_per_item_bonus(p_outlet UUID, p_date DATE)
RETURNS NUMERIC
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT COALESCE(
    (SELECT per_item_bonus FROM public.daily_sales_targets
...
-- 4. Update set_daily_target
DROP FUNCTION IF EXISTS public.set_daily_target(UUID, NUMERIC, NUMERIC);

CREATE OR REPLACE FUNCTION public.set_daily_target(p_outlet UUID, p_amount NUMERIC, p_per_item_bonus NUMERIC DEFAULT 0)
RETURNS void
...
BEGIN
  ...
  END IF;
  INSERT INTO public.daily_sales_targets (outlet_id, target_amount, per_item_bonus, created_by)
  VALUES (p_outlet, p_amount, p_per_item_bonus, auth.uid());
END;
$$;

-- 5. Update get_current_targets
...
-- 6. Update calculate_monthly_crew_bonus
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(SUM(CASE WHEN daily_target_eval.daily_sales >= daily_target_eval.target_amount 
                 THEN (daily_target_eval.additional_items * daily_target_eval.per_item_bonus) 
...
-- 7. Update get_daily_bonus_breakdown
...
BEGIN
  ...
  DailyTargets AS (
    SELECT DISTINCT
      ...
      COALESCE(public.resolve_per_item_bonus(p_outlet_id, d.dt), 0)::NUMERIC AS per_item_bonus
    FROM days d
  ),
  BonusItems AS (
    SELECT 
