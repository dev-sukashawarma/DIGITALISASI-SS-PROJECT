...
export function canComposePO(role: string): boolean {
  return ['admin', 'kitchen', 'purchase', 'developer'].includes(role)
}
export function canVerifyReceipt(role: string): boolean {
  ...
  return ['kitchen', 'admin', 'owner', 'developer'].includes(role)
}
export function canApprovePOFinance(role: string): boolean {
  return ['admin_finance', 'owner', 'admin', 'developer'].includes(role)
}
