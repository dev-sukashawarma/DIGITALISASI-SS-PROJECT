...
export default async function ResepPage() {
  ...
  const [menuRes, recipesRes, channelsRes] = await Promise.all([
    supabase
      .from('menu_items')
      .select('id, name, price, hpp_override, channel_prices, channel_hpp, is_available, is_available_online, available_online_channels, is_package, sort_order, updated_at, updated_by, created_at, categories(name, sort_order), package_items:menu_packages!package_id(id, menu_item_id, quantity)')
      .order('sort_order'),
    supabase
      .from('resep')
      .select('menu_item_ref, is_active, buffer_amount, updated_at, updated_by, created_at, resep_item(id, bahan_baku_id, qty_per_porsi, bahan_baku:bahan_baku_id(id, bahan_baku_sku(harga_beli, qty_isi, is_default, is_active), bahan_baku_harga(harga_beli_display, kemasan_qty)))')
      .eq('scope', 'global'),
    supabase
      .from('sales_channels')
      .select('id, name')
  ...
  // Map recipe raw objects by menu_item_ref
  const recipeObjMap = new Map<string, any>()
  for (const r of recipes) {
    recipeObjMap.set(r.menu_item_ref, r)
  }
  ...
  const hppItems = menuItems.map((menu: any) => {
    ...
    const rObj = recipeObjMap.get(menu.id)
    const updatedAt = rObj?.updated_at || menu.updated_at || rObj?.created_at || menu.created_at || null
    const updatedBy = rObj?.updated_by || menu.updated_by || null
  ...
  const menuWithBOM = menuItems.map((menu: any) => {
    ...
    const bom = !isPackage ? recipes?.find((r: any) => r.menu_item_ref === menu.id) : null
    const updatedAt = bom?.updated_at || menu.updated_at || bom?.created_at || menu.created_at || null
    const updatedBy = bom?.updated_by || menu.updated_by || null
