        ...
        <div className="min-w-0 flex-1 bg-[#fffaf5] lg:rounded-l-[2.5rem]">
          <div className="mx-auto w-full max-w-[1500px] space-y-5 px-4 py-5 sm:px-8 sm:py-7">
            ...
            <section className="grid gap-3 sm:grid-cols-3">
              ...
              <Metric icon={ClipboardX} label="Total rusak" value={findings} tone="orange" />
  ...
  return (
    <article className={`self-start overflow-hidden rounded-[1.75rem] border shadow-lg transition duration-300 hover:-translate-y-1 hover:shadow-2xl ${tone}`}>
      <button type="button" onClick={onToggle} aria-expanded={open} className="relative flex w-full items-center gap-4 overflow-hidden p-5 text-left outline-none focus:ring-4 focus:ring-inset focus:ring-orange-200">
        ...
        <span className="hidden text-right sm:block"><span className={`block text-sm font-black ${issues ? 'text-orange-200' : 'text-emerald-200'}`}>{issues ? `${issues} rusak` : 'Semua baik'}</span><span className="mt-1 block text-[10px] font-bold text-white/50">{review.items.length} item diperiksa</span></span>
        <ChevronDown className={`shrink-0 text-white/60 transition duration-300 ${open ? 'rotate-180' : ''}`} />
      </button>

      {open && <div className="border-t border-white/15 bg-black/10 p-5"><div className="mb-4 grid grid-cols-2 gap-3 sm:hidden"><div className="rounded-xl bg-white/10 p-3 text-center text-xs font-black">{review.items.length} item</div><div className={`rounded-xl p-3 text-center text-xs font-black ${issues ? 'bg-[#f29744] text-[#400a07]' : 'bg-emerald-100 text-emerald-800'}`}>{issues ? `${issues} rusak` : 'Semua baik'}</div></div>{review.note && <p className="mb-4 rounded-2xl border border-white/15 bg-white/10 px-4 py-3 text-sm leading-6 text-white/80"><b className="text-orange-200">Catatan Manager:</b> {review.note}</p>}<div className="grid gap-2 sm:grid-cols-2">{review.items.map((item) => <div key={item.id} className={`rounded-2xl border px-4 py-3 ${item.status === 'issue' ? 'border-orange-300/50 bg-[#f29744] text-[#400a07]' : 'border-white/15 bg-white/10 text-white'}`}><div className="flex items-start justify-between gap-3"><p className="text-sm font-extrabold leading-5">{item.name}</p><span className={`shrink-0 rounded-full px-2 py-1 text-[9px] font-black uppercase ${item.status === 'issue' ? 'bg-[#701604] text-white' : 'bg-emerald-100 text-emerald-800'}`}>{item.status === 'issue' ? 'Rusak' : 'Baik'}</span></div>{item.note && <p className={`mt-2 text-xs leading-5 ${item.status === 'issue' ? 'text-[#4A1713]/75' : 'text-white/60'}`}>{item.note}</p>}</div>)}</div></div>}
    </article>
  )
}
