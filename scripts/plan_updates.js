...
function normalizeOutlet(s) {
  if (!s) return '';
  // Remove common prefixes/suffixes to find the core location name
...
async function main() {
  ...
  for (const ex of excelData) {
    ...
    if (matched) {
      // Already perfectly matched. No name update needed.
      continue;
    }
    ...
    for (const db of dbNames) {
      ...
      if (isCandidate) {
        ...
        // If both are empty after normalization (e.g. both were "KANTOR PUSAT" and "OFFICE"), we can treat them specially if needed.
        // But let's check intersection
        for (const w of exOutWords) {
          if (dbOutWords.includes(w)) outletMatch = true;
        }
        
        // Hardcode BCC == CIMANGGU
  ...
  console.log("=== AKAN DI-UPDATE (Ganti Nama di DB Saja) ===");
  toUpdate.forEach(u => {
    console.log(`[OK] DB "${u.db.original}" -> UPDATE JADI -> "${u.excel.original}" | (Outlet: ${u.excel.location} == ${u.db.outlet})`);
  });

  console.log("\n=== AKAN DITAMBAHKAN SEBAGAI KARYAWAN BARU (Missing / Beda Outlet) ===");
  toInsert.forEach(i => {
    console.log(`[NEW] "${i.original}" | Posisi: ${i.position} | Outlet: ${i.location}`);
  });
}
