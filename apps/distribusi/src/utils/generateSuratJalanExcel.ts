...
export async function downloadSuratJalanExcel(data: SuratJalanExcelData) {
  ...
  const rows: string[] = []
  rows.push(`<row r="1" ht="21">${cell('B1', 'PT SUKA PROFIT BERKAH', 2)}${cell('F1', 'SURAT JALAN', 3)}</row>`)
  ...
  rows.push(`<row r="5" ht="20">${cell('A5', 'Nama Outlet', 5)}${cell('C5', `: ${data.outletName}`, 7)}${cell('E5', 'Nomor Surat Jalan', 5)}${cell('G5', `: ${data.documentNumber}`, 7)}</row>`)
  rows.push(`<row r="6" ht="20">${cell('A6', 'Nomor PO', 5)}${cell('C6', ': -', 7)}${cell('E6', 'Tanggal Surat Jalan', 5)}${cell('G6', `: ${formattedDate}`, 7)}</row>`)
  rows.push(`<row r="7" ht="8"/>`)
  rows.push(`<row r="8" ht="26">${cell('A8', 'No', 8)}${cell('B8', 'Nama Barang', 8)}${cell('E8', 'Satuan', 8)}${cell('F8', 'Jumlah', 8)}${cell('G8', 'Check List', 8)}</row>`)
  ...
  const merges = ['B1:E1', 'B2:E2', 'B3:E3', 'F1:H3', 'A5:B5', 'C5:D5', 'E5:F5', 'G5:H5', 'A6:B6', 'C6:D6', 'E6:F6', 'G6:H6', 'B8:D8', 'G8:H8']
  for (let index = 0; index < itemRows; index++) merges.push(`B${tableStart + index}:D${tableStart + index}`, `G${tableStart + index}:H${tableStart + index}`)
  merges.push(`A${noteStart}:H${noteStart}`, `A${noteStart + 1}:H${noteStart + 3}`, `A${signatureStart}:C${signatureStart}`, `D${signatureStart}:F${signatureStart}`, `G${signatureStart}:H${signatureStart}`)
  for (let row = signatureStart + 1; row <= signatureStart + 4; row++) merges.push(`A${row}:C${row}`, `D${row}:F${row}`, `G${row}:H${row}`)

  const sheet = `${XML_HEADER}<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheetPr><pageSetUpPr fitToPage="1"/></sheetPr><sheetViews><sheetView workbookViewId="0" showGridLines="0"/></sheetViews><cols><col min="1" max="1" width="6" customWidth="1"/><col min="2" max="2" width="25" customWidth="1"/><col min="3" max="4" width="16" customWidth="1"/><col min="5" max="5" width="14" customWidth="1"/><col min="6" max="6" width="13" customWidth="1"/><col min="7" max="7" width="20" customWidth="1"/><col min="8" max="8" width="10" customWidth="1"/></cols><sheetData>${rows.join('')}</sheetData><mergeCells count="${merges.length}">${merges.map(range => `<mergeCell ref="${range}"/>`).join('')}</mergeCells>${logo ? '<drawing r:id="rId1"/>' : ''}<printOptions horizontalCentered="1" verticalCentered="0" gridLines="0"/><pageMargins left="0.18" right="0.18" top="0.2" bottom="0.2" header="0" footer="0.1"/><pageSetup paperSize="9" orientation="landscape" blackAndWhite="1" fitToWidth="1" fitToHeight="1"/><headerFooter><oddFooter>&amp;CPage &amp;P of &amp;N</oddFooter></headerFooter></worksheet>`
  ...
  const files = [
    ...
    { name: 'xl/workbook.xml', content: encoder.encode(`${XML_HEADER}<workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheets><sheet name="Surat Jalan" sheetId="1" r:id="rId1"/></sheets><definedNames><definedName name="_xlnm.Print_Area" localSheetId="0">'Surat Jalan'!$A$1:$H$${printEnd}</definedName></definedNames></workbook>`) },
