...
- Status: Accepted
- Tanggal: 2026-06-19
- Terkait: ADR-002 (reporting hub), ADR-004 (project terpisah), ADR-006 (sinkron Edge Function). Men-supersede pendekatan di spec `2026-06-13-owner-dashboard-penjualan-design.md`.
...
- **Akses:** `crew` (yang menginput food-app) hanya punya akses `absensi` + `stok` — **tidak bisa** membuka owner-dashboard (`ROLE_APP_ACCESS`: owner-dashboard = admin+owner saja).
...
7. **Owner-dashboard membaca satu project** lewat view definer `security_barrier` (pola `monitoring_view_spv`): `sales_summary_spv`, `menu_sales_spv`.
...
- **Form input manual di owner-dashboard** — crew tak punya akses ke app itu.
...
- (+) Owner-dashboard sederhana: satu kontrak data (`orders` hub), satu project dibaca, isolasi dari perubahan skema sumber.
