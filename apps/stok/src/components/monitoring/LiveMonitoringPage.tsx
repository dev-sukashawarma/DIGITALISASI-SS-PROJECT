              ...
              {outlet.lowItems.slice(0, 3).map((item) => (
                <div key={item.bahan_baku_id} className="bg-suka-cream/35 border border-suka-brown/5 rounded-lg py-1.5 px-3 flex items-center justify-between gap-2">
                  ...
                  <p className="text-xs font-black font-mono text-[#ba1a1a] leading-none flex-shrink-0">
                    ...
                    {formatCompositeSaldoAdaptive(item.current_qty, item.saldo_is_gram, '', item.satuan_kecil, item.faktor_tampilan).replace(/\s+/g, ' ').trim()}<span className="text-[10px] font-medium text-suka-brown/50 font-sans">/{item.threshold}</span>
                  </p>
                </div>
              ))}
                  ...
                  >
                    ...
                    <div className="mt-0.5">
                      ...
                      <p className="text-xs font-black font-mono text-[#ba1a1a] mt-0.5 leading-none">
                        {formatCompositeSaldoAdaptive(it.current_qty, it.saldo_is_gram, it.satuan, it.satuan_kecil, it.faktor_tampilan)}/{it.threshold}
                      </p>
                    </div>
                  </div>
                ))}
