    ...
    <div className="fixed inset-0 bg-black/45 backdrop-blur-xs z-50 flex items-center justify-center p-4 animate-in fade-in duration-150">
      <div className="bg-[#fff8f1] w-full max-w-md rounded-2xl border border-[#d9c2b2]/35 p-5 shadow-2xl space-y-4">
        <div className="flex justify-between items-center border-b border-[#d9c2b2]/20 pb-3">
          <h2 className="text-base font-bold text-suka-brown uppercase tracking-tight">Transfer Stok Bahan Baku</h2>
          <button
            onClick={onClose}
            className="w-8 h-8 flex items-center justify-center rounded-full text-suka-brown hover:bg-[#faf2e9] text-base transition-colors cursor-pointer"
            title="Tutup"
          >
            ✕
          </button>
        ...
        <div className="bg-white p-3.5 rounded-xl border border-[#d9c2b2]/35 space-y-1 shadow-xs">
          <p className="text-[10px] text-suka-brown/60 uppercase font-bold tracking-wider">Tujuan Transfer</p>
          <p className="font-bold text-gray-900 text-sm uppercase">{item.outlet_name}</p>
          <p className="text-xs text-suka-brown">
            Bahan: <span className="font-bold text-suka-orange">{item.item_name}</span> (Stok Saat Ini: {item.current_qty} {item.satuan})
          </p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-1.5">
            <label className="block text-[10px] font-bold text-suka-brown uppercase tracking-wider">PILIH OUTLET ASAL (SUMBER)</label>
            <select
              ...
              className="w-full border border-[#d9c2b2]/45 rounded-xl p-2.5 text-sm bg-white text-gray-900 focus:outline-none focus:ring-1 focus:ring-suka-orange focus:border-suka-orange shadow-xs cursor-pointer"
              required
            >
          ...
          <div className="space-y-1.5">
            <label className="block text-[10px] font-bold text-suka-brown uppercase tracking-wider">JUMLAH TRANSFER</label>
            <div className="flex items-center gap-2">
              <input
                ...
                placeholder="0.00"
                className="flex-1 border border-[#d9c2b2]/45 rounded-xl p-2.5 text-sm bg-white text-gray-900 focus:outline-none focus:ring-1 focus:ring-suka-orange focus:border-suka-orange shadow-xs"
                required
              />
              <span className="text-sm font-bold text-suka-brown/70 bg-[#faf2e9] px-3 py-2 rounded-xl border border-[#d9c2b2]/30 capitalize">
                {item.satuan || 'kg'}
              </span>
            </div>
          </div>
          ...
          <div className="flex gap-3 justify-end pt-2">
            <button
              type="button"
              onClick={onClose}
              className="px-4 py-2 border border-[#d9c2b2]/50 rounded-xl text-xs font-bold text-suka-brown bg-white hover:bg-[#faf2e9] active:scale-95 transition-all shadow-xs cursor-pointer"
            >
              Batalkan
            </button>
            <button
              type="submit"
              disabled={!isValid}
              className={`px-4 py-2 rounded-xl text-xs font-bold text-white transition-all shadow-xs active:scale-95 cursor-pointer ${
                isValid
                  ? 'bg-suka-orange hover:bg-orange-600'
