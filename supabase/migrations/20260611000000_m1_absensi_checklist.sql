...
-- Removed redundant index on daily_checklist_records(outlet_id, date) as UNIQUE constraint creates one implicitly
CREATE INDEX idx_daily_checklist_ticks_record_id ON daily_checklist_ticks(record_id);
CREATE INDEX idx_daily_checklist_ticks_item_id ON daily_checklist_ticks(item_id); -- Added index for item_id cascade delete
