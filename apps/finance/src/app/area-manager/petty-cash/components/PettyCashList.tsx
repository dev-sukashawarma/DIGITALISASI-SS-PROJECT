...
export function PettyCashList({ initialRequests }: { initialRequests?: PettyCashTopup[] }) {
  ...
  const reviewRequests = allRequests?.filter(r => r.status === 'forwarded_to_area_manager' || r.status === 'forwarded_by_leader') || []
  const historyRequests = allRequests?.filter(r => r.status !== 'forwarded_to_area_manager' && r.status !== 'forwarded_by_leader') || []
  ...
  const handleForward = async (req: PettyCashTopup) => {
    if (confirm('Anda yakin ingin meneruskan dana ini ke Leader?')) {
              ...
              {requests.map((req) => (
                <tr key={req.id} className="border-b border-suka-gray-100 last:border-0 hover:bg-suka-gray-50 transition-colors">
                  ...
                  <td className="py-3 px-4">
                    ...
                    {req.status === 'forwarded_to_area_manager' && <Badge variant="warning">Menunggu Area Manager</Badge>}
