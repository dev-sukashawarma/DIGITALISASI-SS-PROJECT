...
const ACCENT: Record<Accent, { icon: string; iconBg: string; sub: string }> = {
  ...
  green: { icon: 'text-emerald-600', iconBg: 'bg-emerald-500/10', sub: 'text-emerald-600' },
        ...
        <div className="flex items-center gap-1.5">
          ...
          {tooltip && (
            <div className="group relative flex items-center">
              ...
              <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-64 p-2 bg-suka-brown text-white text-xs rounded-lg shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50 pointer-events-none text-center leading-relaxed">
                {tooltip}
                <div className="absolute top-full left-1/2 -translate-x-1/2 border-4 border-transparent border-t-suka-brown"></div>
              </div>
            </div>
          )}
        </div>
