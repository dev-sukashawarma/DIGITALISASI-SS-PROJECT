...
async function main() {
  // Try to fetch all functions via a rest call to introspection endpoints? 
  // No, Supabase doesn't expose pg_proc via REST API. 
  // Maybe we can fetch from pg_proc using a generic select if it is exposed.
  // It's usually not exposed to postgrest.
  const { data, error } = await supabase.from('pg_proc').select('*').limit(5);
  console.log('pg_proc query:', data || error.message);
}
