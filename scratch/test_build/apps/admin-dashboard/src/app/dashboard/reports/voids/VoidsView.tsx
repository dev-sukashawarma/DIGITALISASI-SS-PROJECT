...
export default function VoidsView({ initialVoids }: { initialVoids: any[] }) {
  ...
  useEffect(() => {
    const channel = supabase.channel('realtime_voids')
      .on(
        'postgres_changes',
        { event: 'UPDATE', schema: 'public', table: 'orders' },
        (payload) => {
          if (payload.new.status === 'cancelled') {
            toast.error('Ada Pembatalan Pesanan Baru!', {
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">
          Monitor pembatalan pesanan dan aktivitas mencurigakan di kasir secara realtime.
        </p>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            ...
            <tbody className="divide-y">
              {!initialVoids || initialVoids.length === 0 ? (
                <tr>
