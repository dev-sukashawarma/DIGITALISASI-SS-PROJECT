...
async function checkCibuburOrders() {
  ...
  if (!cibuburOutlet) {
    console.log("Outlet Cibubur not found. Available outlets:")
    outlets?.forEach(o => console.log(`- ${o.name} (${o.id})`))
    return
  }

  console.log(`Outlet found: ${cibuburOutlet.name} (${cibuburOutlet.id})`)
  ...
  console.log(`\n=== PESANAN TIKTOK GO DI CIBUBUR TANGGAL 13 AGUSTUS 2026 ===`)
  if (!orders || orders.length === 0) {
    ...
    console.log(`\nGRAND TOTAL CIBUBUR: Rp ${grandTotal.toLocaleString('id-ID')}`)
