...
async function main() {
  ...
  // Find the exact ones we're talking about
  const targetOutlets = [
    'SS KITCHEN', 'SS SAWANGAN', 'SS PAJAJARAN', 'SS JATIWARINGIN',
    'SS CIRENDEU', 'SS DRAMAGA', 'SS CIBINONG', 'CIBUBUR', 'SENTUL',
    'CICURUG', 'CILEUNGSI'
  ];

  const targetList = excelData.filter(ex => targetOutlets.includes(ex.location));

  console.log("=== ANALISIS KEMUNGKINAN KARYAWAN PINDAH OUTLET ATAU BARU ===\n");
  
  for (const ex of targetList) {
    let exactMatch = dbNames.find(db => db.normalized === ex.normalized);
    if (exactMatch) continue; // Already handled as perfect match

    // Look for strong fuzzy matches (name only) ignoring outlet
    let bestFuzzy = null;
    let minDistance = 999;
    ...
    if (bestFuzzy) {
      console.log(`? PINDAH OUTLET / NAMA MIRIP?`);
      console.log(`  Excel: "${ex.original}" di ${ex.location}`);
      console.log(`  DB   : "${bestFuzzy.original}" di ${bestFuzzy.outlet}`);
      console.log();
    } else {
      console.log(`+ KARYAWAN BARU (Tidak ada jejak di DB)`);
      console.log(`  Excel: "${ex.original}" di ${ex.location}`);
      console.log();
    }
  }
