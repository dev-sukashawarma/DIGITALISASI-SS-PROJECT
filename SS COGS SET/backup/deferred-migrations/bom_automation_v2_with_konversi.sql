-- DRAFT v2 -- BELUM di-push, untuk direview dulu.
-- Perubahan dari versi asli (20260703000000_bom_automation.sql):
--   Baris pemotongan stok sekarang membagi dengan bahan_baku.faktor_konversi
--   (kolom baru dari migration 20260704160000) supaya qty_per_porsi (satuan resep,
--   mis. gram/lembar/cm) dikonversi dulu ke satuan_baku (kg/pack/pcs/dst) SEBELUM
--   dipotong dari ledger_stok. Tanpa ini, potongan stok bisa salah ratusan-ribuan x
--   (mis. AYAM 100 gram terpotong sbg 100 KG kalau tak dikonversi).
-- Bagian void/reversal TIDAK berubah -- dia cuma membalik angka yg SUDAH tersimpan
-- di ledger_stok (satuan_baku), jadi tidak perlu konversi ulang.
...
-- 2. Create Trigger Function for Automatic BOM Deduction (v2: konversi satuan)
