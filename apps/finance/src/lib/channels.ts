...
export function getChannel(id: string | null | undefined): ChannelConfig | null {
  ...
  // TikTok Shop (SS Online E-Commerce)
  if (['tiktokshop', 'tiktok_shop', 'f3305089-b9e4-4b92-95da-14bf6e7fb6d5'].includes(norm)) {
    return CHANNELS.find((c) => c.id === 'tiktok_shop') ?? null
  }

  // Shopee Seller (SS Online E-Commerce)
  if (['shopee_shop', 'shopeeseller', 'shopee_seller', 'd68eb5ec-d6bb-4d0a-8758-a2600c8f1584'].includes(norm)) {
    return CHANNELS.find((c) => c.id === 'shopee_shop') ?? null
  }

  // TikTok Go (Food Delivery)
  ...
  // SukaShawarma APP (aplikasi pelanggan). `susunPayloadPos` di retail-gateway
  // menulis channel/sales_source = 'app'; sisanya alias untuk data manual.
  if (['app', 'aplikasi', 'suka_app', 'sukashawarma_app'].includes(norm)) {
    return CHANNELS.find((c) => c.id === 'app') ?? null
  }

  // Generic fallback for food apps
