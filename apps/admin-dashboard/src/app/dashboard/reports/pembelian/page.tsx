  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-0.5">Realisasi pembelian dari supplier ke Kitchen Pusat.</p>
      </div>

      {/* Filters */}
      <div className="bg-white rounded-2xl border border-suka-gray-200 shadow-sm p-4 flex flex-wrap gap-3 items-center">
        <span className="text-sm font-bold text-gray-500">Periode:</span>
        ...
        <input type="date" value={to} onChange={e => setTo(e.target.value)}
          className="border border-suka-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-suka-brown/20" />
        <div className="ml-auto font-extrabold text-suka-brown text-lg">{rupiah(grandTotal)}</div>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16 text-gray-400 bg-white rounded-2xl border border-suka-gray-200">
          ...
          <p className="text-sm mt-1">Data muncul setelah PO diverifikasi penerimaan</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-2xl border border-suka-gray-200 shadow-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-suka-gray-100">
              <h2 className="font-bold text-suka-brown text-sm uppercase tracking-wide">Total per Bulan</h2>
            </div>
            <div className="divide-y divide-suka-gray-100">
              {bulanList.map(b => (
                <div key={b.bulan} className="px-5 py-3 flex justify-between items-center">
                  ...
                  <div className="font-extrabold text-suka-brown">{rupiah(b.total)}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-suka-gray-200 shadow-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-suka-gray-100">
              <h2 className="font-bold text-suka-brown text-sm uppercase tracking-wide">Top 10 Bahan by Pengeluaran</h2>
            </div>
            <div className="divide-y divide-suka-gray-100">
              {topBahan.map((b, idx) => (
                <div key={b.nama} className="px-5 py-3 flex items-center gap-3">
          ...
          <div className="bg-white rounded-2xl border border-suka-gray-200 shadow-sm overflow-hidden lg:col-span-2">
            <div className="px-5 py-4 border-b border-suka-gray-100">
              <h2 className="font-bold text-suka-brown text-sm uppercase tracking-wide">Tren Harga Rata-Rata Tertimbang per Bahan</h2>
            </div>
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-suka-gray-100">
                  {rows.map((r) => {
                    ...
                    return (
                      <tr key={`${r.bulan}-${r.bahan_baku_id}-${r.supplier_nama}`} className="hover:bg-suka-gray-50 transition-colors">
                        <td className="px-5 py-3 font-medium text-suka-ink">{r.nama_bahan}
                        ...
                        <td className="px-4 py-3 text-right font-bold text-suka-brown">{rupiah(r.total_nilai)}</td>
                      </tr>
                    )
                  })}
                </tbody>
