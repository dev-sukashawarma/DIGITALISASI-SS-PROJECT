...
async function run() {
  ...
  const target = outlets.find(o => o.name.toLowerCase().includes('beji'));
  
  if (!target) {
    console.error('Beji outlet not found');
    return;
  }
  
  console.log('Found Beji outlet:', target.id);
  
  // 2. Get or create record
  let { data: record } = await supabase
    .from('daily_checklist_records')
    .select('id')
    .eq('outlet_id', target.id)
    .eq('date', date)
    .maybeSingle();
  ...
  if (!record) {
    ...
    const { data: inserted, error } = await supabase
      .from('daily_checklist_records')
      .insert({ outlet_id: target.id, date })
      .select('id')
      .single();
  ...
  if (ticksError) {
    ...
    console.log('Successfully bypassed all checklist items for Beji for today!');
  }
}
