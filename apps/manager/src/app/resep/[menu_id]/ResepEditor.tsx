...
export function ResepEditor({ menu, bahanBakuList, existingRecipe, comboBOMData }: any) {
  ...
  const lastUpdaterName = existingRecipe?.updated_by || menu.updated_by
  const lastUpdateTime = existingRecipe?.updated_at || existingRecipe?.created_at || menu.updated_at || menu.created_at

  return (
    <div className="space-y-6">
      {/* Last update notice banner */}
      {(lastUpdaterName || lastUpdateTime) && (
        <div className="flex items-center gap-2.5 px-4 py-2.5 rounded-2xl bg-amber-50/80 border border-amber-200/70 text-xs text-amber-900 shadow-sm">
          <UserCheck className="w-4 h-4 text-suka-orange flex-shrink-0" />
          <span>
            Terakhir diperbarui oleh: <strong className="text-suka-brown font-black">{lastUpdaterName || 'Admin'}</strong>
            {lastUpdateTime ? ` pada ${new Date(lastUpdateTime).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit', timeZone: 'Asia/Jakarta' })}` : ''}
          </span>
        </div>
      )}
