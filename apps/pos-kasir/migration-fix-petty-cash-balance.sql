-- Fix get_petty_cash_balance to correctly include topups that were completed during the shift
-- even if they were created before the shift started.
...
DECLARE
  ...
  v_shift_id UUID;
BEGIN
  -- Ambil shift yang sedang BUKA (open)
  ...
  IF v_shift_id IS NOT NULL THEN
    -- Topup selama shift ini (created ATAU completed di dalam shift ini)
    ...
    WHERE outlet_id = p_outlet_id 
      ...
      AND (created_at >= v_shift_start OR completed_at >= v_shift_start);
