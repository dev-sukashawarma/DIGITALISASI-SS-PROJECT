  ...
  try {
    ...
    if (insertError) {
      ...
      if (insertError.code === '23505' || insertError.message.includes('outlet_staff_nik_key')) {
        return { ok: false, error: `NIK "${cleanNik}" sudah terdaftar pada karyawan lain.` }
      }
      ...
      if (insertError.code === '23505' || insertError.message.includes('outlet_staff_username_key')) {
        return { ok: false, error: `Username "${username}" sudah terdaftar pada karyawan lain.` }
      }
    ...
    if (
      ...
      if (finError) {
        console.warn('Gagal menyimpan data finansial:', finError.message);
      }
    }
