...
- [x] **Step 1: Delete the stale duplicate config**
...
- [x] **Step 2: Rewrite next.config.ts to the clean Node-server form**
...
- [x] **Step 3: Remove the stale static-export artifact**
...
- [x] **Step 4: Verify config is valid (build starts, no eslint-key error)**
...
- [x] **Step 1: Add baseUrl to tsconfig**
...
- [x] **Step 2: Re-run type-check to get the real (post-baseUrl) error list**
...
- [x] **Step 3: Annotate Supabase callback params (apply this mapping)**
...
- [x] **Step 4: Verify type-check is clean**
...
- [x] **Step 5: Verify build succeeds**
...
- [x] **Step 1: Confirm no remaining server-client usage before deleting**
...
- [x] **Step 2: Replace the import in every consumer**
...
- [x] **Step 3: Replace every call site**
...
- [x] **Step 4: Delete the local supabase module (browser dup + service-role footgun)**
...
- [x] **Step 5: Verify no dangling imports**
...
- [x] **Step 6: Verify type-check + build**
...
- [x] **Step 2: Find every direct date-format render to migrate**
...
- [x] **Step 3: Migrate inline renders to the helper**
...
- [x] **Step 4: Verify type-check + build**
...
- [x] **Step 1: Add scripts + devDependencies to package.json**
...
- [x] **Step 5: Add the vitest/jest-dom types to tsconfig**
...
- [x] **Step 6: Write a real test for the date helper**
...
- [x] **Step 8: Verify type-check still clean**
...
- [x] `cd apps/distribusi && rm -f tsconfig.tsbuildinfo && yarn type-check` → exit 0, no `error TS`.
- [x] `cd apps/distribusi && yarn build` → completes, no `out/` dir, no eslint-key error.
- [x] `grep -rn "@/lib/supabase\|createClient()" apps/distribusi/src` → no matches.
- [x] (if Task 5 done) `cd apps/distribusi && yarn test` → all pass.
- [x] Manual smoke (when re-uploading): SSO login works cross-subdomain, `middleware.ts` guard runs, surat-jalan create + verifikasi penerimaan flow intact.
