                      ...
                      <React.Fragment key={menu.id}>
                        ...
                        <tr className="group hover:bg-white/80 hover:shadow-lg hover:shadow-amber-500/5 hover:-translate-y-0.5 hover:scale-[1.005] transition-all duration-300 relative z-10 hover:z-20 border-b border-gray-50">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            {menu.updatedBy || menu.updatedAt ? (
                              <div className="flex flex-col">
                                <span className="text-xs font-bold text-gray-900 flex items-center gap-1.5">
                                  <span className="w-1.5 h-1.5 rounded-full bg-suka-orange inline-block" />
                                  {menu.updatedBy || 'Admin'}
                                </span>
                                <span className="text-[10px] text-gray-400 font-semibold">
                                  {menu.updatedAt ? new Date(menu.updatedAt).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit', timeZone: 'Asia/Jakarta' }) : '—'}
                                </span>
                              </div>
                            ) : (
