...
export default function PromoDailyScheduleEditor({
  ...
  return (
    <div className={`rounded-2xl border p-4 space-y-3 ${style.box}`}>
      <label className="flex items-start gap-3 cursor-pointer group">
        <div className="relative flex items-center pt-0.5">
          ...
          />
          <div
            className={`w-5 h-5 rounded-md border-2 bg-white transition-all flex items-center justify-center ${style.border.split(' ')[0]} ${style.icon} peer-checked:bg-current peer-checked:border-current`}
          >
            <Check
              className="w-3.5 h-3.5 text-white opacity-0 peer-checked:opacity-100 scale-50 peer-checked:scale-100 transition-all"
              strokeWidth={3}
            />
          </div>
        </div>
        <div className="flex-1">
          <span className="flex items-center gap-2 text-sm font-bold text-stone-800 group-hover:text-stone-700 transition-colors">
      ...
      {enabled && (
        <div className="pl-8 space-y-2">
          {rows.map((row, index) => (
            <div
              key={`${index}-${row.date}`}
              className="grid grid-cols-1 sm:grid-cols-[minmax(8rem,1.2fr)_minmax(6rem,1fr)_minmax(6rem,1fr)_auto] items-end gap-2"
            >
              <label className="space-y-1">
                <span className="block text-[11px] font-bold text-stone-600">Tanggal (WIB)</span>
                <input
                  type="date"
                  value={row.date}
                  onChange={(event) => updateRow(index, { date: event.target.value })}
                  className={`w-full rounded-xl border border-stone-200 bg-white px-2.5 py-1.5 text-sm font-semibold text-stone-900 outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500`}
                />
              </label>
              <label className="space-y-1">
                <span className="block text-[11px] font-bold text-stone-600">Mulai</span>
                <input
                  type="time"
                  value={timeValue(row.start_time, '17:00')}
                  onChange={(event) =>
                    updateRow(index, { start_time: event.target.value ? `${event.target.value}:00` : '' })
                  }
                  className={`w-full rounded-xl border border-stone-200 bg-white px-2.5 py-1.5 text-sm font-semibold text-stone-900 outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500`}
                />
              </label>
              <label className="space-y-1">
                <span className="block text-[11px] font-bold text-stone-600">Selesai</span>
                <input
                  type="time"
                  value={timeValue(row.end_time, '20:00')}
                  onChange={(event) =>
                    updateRow(index, { end_time: event.target.value ? `${event.target.value}:00` : '' })
                  }
                  className={`w-full rounded-xl border border-stone-200 bg-white px-2.5 py-1.5 text-sm font-semibold text-stone-900 outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500`}
                />
              </label>
              <button
                ...
                className="mb-0.5 rounded-xl border border-stone-200 p-2 text-stone-400 hover:border-rose-200 hover:bg-rose-50 hover:text-rose-600 transition-colors cursor-pointer"
              >
          ...
          <button
            type="button"
            onClick={addRow}
            className={`inline-flex items-center gap-1.5 rounded-xl border px-3 py-2 text-xs font-bold transition-colors cursor-pointer ${style.button}`}
          >
