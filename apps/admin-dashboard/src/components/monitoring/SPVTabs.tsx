...
export function SPVTabs({ 
  ...
  const allTabs: { id: SPVTabId; label: string; count: number | null }[] = [
    ...
    { id: 'harga_bahan', label: 'Master Bahan Baku', count: null },
    ...(showPOInbound ? [{ id: 'po_inbound' as SPVTabId, label: 'Purchase Order', count: poInboundCount || 0 }] : []),
  ];
  ...
  return (
    <nav className="bg-white border-b border-suka-brown/10 px-4 md:px-6 flex items-center gap-2 overflow-x-auto scrollbar-none whitespace-nowrap py-2 rounded-t-2xl">
      {tabs.map((tab) => {
