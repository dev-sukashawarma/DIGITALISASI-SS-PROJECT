...
async function run() {
  ...
  if (beji) {
    const { data: attendance } = await supabase.from('attendance')
      ...
      .gte('ts_server', '2026-07-30T00:00:00Z')
      .order('ts_server', { ascending: false });
      
    console.log('Attendance at Beji today:');
    if (attendance) {
      for (const a of attendance) {
        console.log('- ' + a.outlet_staff.name + ' (' + a.outlet_staff.role + ') type:' + a.type + ' current_outlet_id:' + a.outlet_staff.outlet_id);
