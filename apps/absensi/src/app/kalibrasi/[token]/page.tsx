  ...
  const recordLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        ...
        try {
          const res = await fetch(`/api/kalibrasi/submit`, {
            method: 'POST',
      ...
      <div className="max-w-md w-full bg-white rounded-3xl shadow-xl border border-gray-100 overflow-hidden">
        <div className="bg-suka-orange p-6 text-center text-white">
          ...
          <p className="text-white/80 text-sm mt-2 font-medium">
            Sistem Pembaruan Koordinat GPS
          </p>
        </div>
        ...
        <div className="p-8 text-center space-y-6">
          {status === "idle" && (
            <>
              <p className="text-gray-600 text-sm leading-relaxed">
                Pastikan Anda sedang berdiri **tepat di area outlet**. Tekan tombol di bawah ini untuk merekam dan mengirimkan titik lokasi Anda saat ini kepada SPV.
              </p>
              <button 
          ...
          {status === "loading" && (
            <div className="py-8 space-y-4">
              ...
              <p className="text-suka-orange font-bold animate-pulse">Mendeteksi koordinat GPS presisi...</p>
            </div>
          )}
          ...
          {status === "success" && (
            <div className="py-4 space-y-4">
              ...
              <div>
                <h3 className="font-black text-xl text-suka-ink">Berhasil!</h3>
                <p className="text-gray-500 mt-2 text-sm leading-relaxed">
                  Titik lokasi Anda telah berhasil direkam dan diteruskan ke SPV. Anda boleh menutup halaman ini.
                </p>
              </div>
            </div>
          )}
