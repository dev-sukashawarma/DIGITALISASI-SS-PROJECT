...
async function resyncAllToday() {
  ...
  const { data: orders } = await supabase
    ...
    .select('*, order_items(*)')
    .eq('status', 'completed')
    .gte('created_at', '2026-07-21T17:00:00Z') // Local WIB 00:00:00
    .lt('created_at', '2026-07-22T17:00:00Z'); // Local WIB 23:59:59

  if (!orders || orders.length === 0) return console.log('No orders to sync.');

  console.log('Resyncing ' + orders.length + ' orders to Google Sheets for all outlets...');
  ...
  for (const order of orders) {
    ...
    const payload = {
      ...
      day_of_month: 22, // specific day to force placement
    ...
    try {
      const response = await fetch(webhookUrl, { method: 'POST', headers: { 'Content-Type': 'text/plain;charset=utf-8' }, body: JSON.stringify(payload), redirect: 'follow' });
      if (response.ok) {
        successCount++;
  ...
  console.log('\nSuccessfully synced ' + successCount + ' out of ' + orders.length + ' orders for ALL OUTLETS.');
}
resyncAllToday();
