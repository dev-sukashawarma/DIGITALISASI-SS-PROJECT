              ...
              <div className="bg-white rounded-2xl border border-suka-gray-200 shadow-sm overflow-hidden">
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-xs">
                    <thead>
                      <tr className="bg-amber-50/50 text-left text-gray-500 font-bold border-b border-suka-gray-100">
                    ...
                    <tbody className="divide-y divide-suka-gray-100 font-medium">
                      {byOutlet.length === 0 ? (
                        <tr><td colSpan={4} className="py-8 text-center text-gray-400">Belum ada waste pada periode ini</td></tr>
                      ) : byOutlet.map(o => {
                        const budget = budgetByOutlet.get(o.id) ?? 0
                        const rowGap = computeWasteGap(o.nilai, budget)
                        return (
                          <tr key={o.id}>
                            <td className="py-3 px-6 text-suka-brown font-bold">{o.name.replace('SUKA SHAWARMA ', '')}</td>
                            <td className="py-3 px-6 text-right text-rose-700 font-extrabold">{rupiah(o.nilai)}</td>
                            <td className="py-3 px-6 text-right text-gray-600">{rupiah(budget)}</td>
                            <td className={`py-3 px-6 text-right font-bold ${rowGap.gapPct === null ? 'text-gray-400' : rowGap.gapPct > 0 ? 'text-rose-700' : 'text-emerald-600'}`}>
                ...
                <table className="w-full text-xs">
                  ...
                  <tbody className="divide-y divide-suka-gray-100 font-medium">
                    {byBahanAndReason.length === 0 ? (
                      <tr><td colSpan={6} className="py-8 text-center text-gray-400">Belum ada waste pada periode ini</td></tr>
                      ...
                      <tr key={b.id}>
                        ...
                        <td className="py-3 px-6 text-gray-900 font-bold">{b.bahan_nama}</td>
                        <td className="py-3 px-6 text-gray-600">{b.reason}</td>
                        <td className="py-3 px-6 text-right text-gray-600">{b.qty_kecil} {b.satuan_kecil}</td>
                        <td className="py-3 px-6 text-right text-gray-600">{rupiah(b.hpp_kecil)}</td>
                        <td className="py-3 px-6 text-right text-rose-700 font-extrabold">{rupiah(b.nilai)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
