...
export default function TargetsView({ initialTargets, initialGlobalDefault, initialGlobalDefaultBonus, initialHistory }: TargetsViewProps) {
  ...
  const isReadOnly = false // Owner is always rw in owner-dashboard
    ...
    try {
      ...
      if (hasMessage) {
        const { error } = await supabase.rpc('send_owner_message', {
          ...
          p_outlet_ids: audienceAll ? null : Array.from(selectedOutlets),
    ...
    <div className="space-y-6">
      <div className="bg-white p-4 rounded-2xl border border-suka-gray-200 shadow-sm">
        <div className="flex items-center gap-3">
          <h2 className="text-xl font-extrabold text-suka-brown tracking-tight">Target & Pesan</h2>
        ...
        <p className="text-xs text-suka-gray-500 font-medium mt-1">Atur target penjualan harian dan kirim pesan ke kasir — sekaligus atau salah satu.</p>
      </div>
