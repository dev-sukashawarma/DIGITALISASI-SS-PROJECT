...
// Salinan apps/stok/src/components/permintaan/PilihVendorBahan.tsx — ubah keduanya.
import type { Alokasi, SaldoVendor } from '@/lib/alokasiVendor'
  ...
  return (
    <div className="mt-2 rounded-lg border border-suka-brown/15 p-2 space-y-1 text-[11px]">
          ...
          <label key={v.vendor_id} className={`flex items-center gap-2 ${habis ? 'opacity-50' : ''}`}>
            ...
            {pecah && a && (
              <input type="number" min="0" step="any" disabled={disabled} value={a.qty || ''}
                ...
                className="w-16 rounded border border-suka-brown/20 p-0.5 text-right" />
            )}
          </label>
        )
      })}
      ...
      {!pecah && alokasi.length > 0 && !disabled && (
        <button type="button" className="ml-2 text-suka-orange underline"
