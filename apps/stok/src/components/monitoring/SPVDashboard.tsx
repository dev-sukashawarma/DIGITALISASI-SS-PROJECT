...
export function SPVDashboard({ allowedOutletIds }: { allowedOutletIds?: string[] } = {}) {
  ...
  React.useEffect(() => {
    if (visibleOutlets.length > 0 && !selectedOutletId) {
      ...
      const isKitchenRole = outletStaff?.role === 'kitchen' || outletStaff?.role === 'purchasing' || outletStaff?.role === 'admin' || outletStaff?.role === 'admin_hr' || outletStaff?.role === 'admin_finance' || outletStaff?.role === 'developer';
      if (isKitchenRole) {
